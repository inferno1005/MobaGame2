...
namespace MobaGame2
{
    class Minion : NewChamp
    {
        ...
        {
            this.Name = "Minion";
            this.texture.name= "texture\\Minion";

            this.texture.height = 32;
            this.texture.width = 32;
            this.position.position = new Vector2(10, 10);
            ...
            this.abilities[0].texture.name= "texture\\fireball";
            enemybase = ge;
        }
        public void Agro(GameEntity target)
        {
            this.position.Distance();
            if (this.attribute.range > this.position.distance)
            {
                FocusObject(target);
            }
        }
